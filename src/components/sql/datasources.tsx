...
// Lucide React icons
import { Database, Upload, FileText, Table2, Eye, Link, X, Search, BarChart3, Shield, Lightbulb, CheckCircle, XCircle, Loader2, HelpCircle } from 'lucide-react'
...
export interface DataSourcesProps {
  ...
  onExecuteAnalyticalQuery?: ((query: AnalyticalQuery, dataSource?: DataSource) => Promise<void>) | undefined
...
export function DataSources({
  ...
  const handleDataSourceClick = (dataSource: DataSource): void => {
    ...
    if (dataSource.loadingStatus === 'failed' || dataSource.loadingStatus === 'loading') {
      return
    }
  ...
  const handleAnalyticalQuery = async (query: AnalyticalQuery, dataSource: DataSource, event: React.MouseEvent): Promise<void> => {
  ...
  const handleRemoveDataSource = (dataSource: DataSource, event: React.MouseEvent): void => {
    event.stopPropagation()
    event.preventDefault()
  ...
  const hasLoadingDataSources = React.useMemo(() => {
    return isLoadingBatch || dataSources.some(ds => ds.loadingStatus === 'loading')
    ...
    <div className={cn('relative bg-background', className)}>
      ...
      >
        ...
        {hasLoadingDataSources && (
          <span className="absolute top-0 right-0 flex h-2 w-2" key="loading-indicator">
            ...
            {filteredDataSources.length === 0 ? (
              ...
              <div role="menu">
                {filteredDataSources.map((dataSource, index) => {
                  ...
                  const hasAnalysis = onExecuteAnalyticalQuery && availableQueries.length > 0
                  const isDisabled = dataSource.loadingStatus === 'failed' || dataSource.loadingStatus === 'loading' || dataSource.loadingStatus === 'verification_needed'
                  const isFailed = dataSource.loadingStatus === 'failed'
                  const isLoading = dataSource.loadingStatus === 'loading'
                  const needsVerification = dataSource.loadingStatus === 'verification_needed'
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center mt-1 gap-1 text-xs text-muted-foreground">
                                <span>Table:</span>
                                <span className='font-mono bg-muted inline-block px-1'>{dataSource.tableName}</span>
                                {/* Status checkmark */}
                                {isLoading && (
                                  <span title="Loading...">
                                ...
                                {needsVerification && (
                                  <span title="Verification needed" className="animate-pulse">
                                ...
                                {isFailed && (
                                  <span
                                    ...
                                    title={dataSource.loadingError || 'Failed to load'}
                                  >
                        ...
                        {isFailed && onRemoveDataSource && (
                          <div className="px-2">
                            <Button
                              ...
                              onClick={(e) => handleRemoveDataSource(dataSource, e)}
                              ...
                              <DropdownMenuContent align="end" className="w-56">
                                ...
                                {availableQueries.map((query) => {
                                  const renderIcon = () => {
                                    ...
                                    switch (query.icon) {
                                      ...
                                      default:
                                        ...
                                        return <span className="h-4 w-4 flex-shrink-0 flex items-center justify-center text-sm">{query.icon}</span>
                                    }
                                  }
                                  ...
                                  return (
                                    <DropdownMenuItem
                                      key={query.id}
                                      onClick={(e) => handleAnalyticalQuery(query, dataSource, e)}
                                      onMouseDown={(e) => handleAnalyticalQuery(query, dataSource, e)}
