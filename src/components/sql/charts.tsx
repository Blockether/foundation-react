...
import { AnalyticalQuery, QueryResult, AnalyticalQueryCategory } from '@/types/sql'
 ...
 */
const processChartData = (result: QueryResult, category: AnalyticalQueryCategory) => {
...
const processInsightsData = (data: any[]) => {
  return data.map((row: any) => ({
    ...
    ...(Object.fromEntries(
      Object.entries(row).filter(([key]) => key.includes('_top_values')).map(([key, value]) => {
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {data.map((row: any, index: number) => (
          <div
            key={index}
            className="bg-muted/30 border rounded-md p-4"
          >
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {data.map((row: any, index: number) => (
          <div key={index} className="bg-gradient-to-br from-primary/5 to-primary/10 border border-primary/20 rounded-md p-4">
            <h5 className="text-sm font-medium text-muted-foreground mb-1">{row.table}</h5>
      ...
      {data.map((row: any, rowIndex: number) => {
        const topValueKeys = Object.keys(row).filter(key => key.startsWith('top_'))
          ...
          <div key={rowIndex} className="space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {topValueKeys.map((key: string, colIndex: number) => {
                ...
                return (
                  <div key={colIndex} className="bg-muted/20 border rounded-md p-3">
                    <h6 className="text-xs font-medium text-muted-foreground mb-2">
                      {columnName}
                    </h6>
                    <div className="space-y-1">
                      {values.slice(0, 3).map((value: string, valIndex: number) => (
                        <div key={valIndex} className="text-xs truncate" title={value}>
                          • {value}
                        </div>
                      ))}
                    </div>
...
const processBenchmarkingData = (data: any[]) => {
  ...
  return data.map((row) => ({
    ...
    performance: Number((row.current_value || row.current) / (row.benchmark_value || row.benchmark) * 100)
  }))
}
...
const processHealthData = (data: any[]) => {
  ...
  return data.map((row) => ({
    ...
    weightedScore: Number(row.score || row.rating) * Number(row.weight || row.importance)
  }))
}
...
const CustomCategoryChart = ({ data, category }: { data: any[], category: string }) => (
  <div className="space-y-4">
    ...
    {data.length > 0 ? (
      <div className="bg-muted/20 border rounded-md p-4">
        ...
        <div className="text-xs">
          <p><strong>Data shape:</strong> {data.length} rows × {Object.keys(data[0] || {}).length} columns</p>
 ...
 */
export function AnalyticalChart({ query, result, className }: AnalyticalChartProps): React.ReactNode {
  ...
  const renderChart = () => {
    switch (query.category) {
      ...
      default:
        ...
        return <CustomCategoryChart data={chartData} category={query.category} />
    }
  }
