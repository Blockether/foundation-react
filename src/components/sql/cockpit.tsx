...
import { SavedQuery, QueryResult, SQLError, DataSource, AnalyticalQuery, SQLCockpitProps } from '@/types/sql'
import { useDuckDB, useDuckDBConnection, transformDuckDBResult, transformErrorToSQLError } from '@/lib/duckdb'
...
const SQLCockpitWrappedContent = ({
  ...
  const [queryState, setQueryState] = useState<DuckDBQueryState>(DuckDBQueryState.QueryIdle)
  ...
  const [currentAnalyticalQuery, setCurrentAnalyticalQuery] = useState<AnalyticalQuery | null>(null)

  // SQL formatter
  const { formatQuery: formatQueryAsync, lastError: formatError, clearError: clearFormatError } = useSQLFormatter({
    dialect: 'sql',
    keywordCase: 'upper',
  })
  ...
  const getTableSchema = useCallback(async (tableName: string): Promise<DataSource['schema']> => {
  ...
  // Handle file import
  const handleImportFile = useCallback(async (file: File, existingDataSourceId?: string): Promise<void> => {
    if (!connection) {
    ...
    try {
      ...
      if (existingDataSourceId) {
        const existingDataSource = dataSources.find(ds => ds.id === existingDataSourceId)
        tableName = existingDataSource?.tableName || file.name.replace(/\.[^/.]+$/, '').replace(/[^a-zA-Z0-9_]/g, '_')
      } else {
  ...
  const handleExecuteAnalyticalQuery = useCallback(async (analyticalQuery: AnalyticalQuery, dataSource?: DataSource): Promise<void> => {
    if (!connection) {
    ...
    try {
      ...
      if (dataSource) {
        queryToExecute = analyticalQuery.query.replace(/\busers\b/g, dataSource.tableName)
        console.log(`Substituted table name: "${dataSource.tableName}"`, queryToExecute)
      }

      // Execute the analytical query
      const result = await transformDuckDBResult(connection, queryToExecute, startTime)
      setQueryResult(result)
      setQueryState(DuckDBQueryState.QueryCompleted)

      toast.success(
        <span>
          {analyticalQuery.name} on table <TableName>{dataSource?.tableName || 'unknown'}</TableName> completed successfully
        </span>
      )
    } catch (err) {
      setQueryError(transformErrorToSQLError(err))
      ...
      toast.error(
        <span>
          Analytical query on table <TableName>{dataSource?.tableName || 'unknown'}</TableName> failed: {err instanceof Error ? err.message : 'Unknown error'}
        </span>
      )
    }
  }, [connection])
  ...
  React.useEffect(() => {
    if (!connection || !db) {
      console.log('[DataSourceLoader] Waiting for DuckDB connection...', { connection: !!connection, db: !!db })
      return
    }
    ...
    const loadInitialDataSources = async () => {
      ...
      const newDataSources = initialDataSources.filter(ds => {
        const isActuallyLoaded = loadedDataSourceIdsRef.current.has(ds.id) && ds.loadingStatus === 'loaded'
      ...
      for (let index = 0; index < newDataSources.length; index++) {
        ...
        try {
          console.log(`[DataSourceLoader] Processing: ${dataSource.name} (${dataSource.tableName})`)

          // Skip if the table already exists (e.g., manually created or rehydrated state)
          try {
            await connection.query(`SELECT 1 FROM ${dataSource.tableName} LIMIT 1`)
            console.log(`[DataSourceLoader] Table ${dataSource.tableName} already exists, skipping load but marking as loaded`)
            ...
            console.log(`[DataSourceLoader] Table ${dataSource.tableName} does not exist, proceeding with load`)
          }

          // Load from URL
          if (dataSource.url) {
            console.log(`[DataSourceLoader] Fetching from URL: ${dataSource.url}`)
            const response = await fetch(dataSource.url)

            if (!response.ok) {
              throw new Error(`Failed to fetch ${dataSource.url}: ${response.status} ${response.statusText}`)
            }
            ...
            console.log(`[DataSourceLoader] Importing file: ${fileName} (${blob.size} bytes)`)
            ...
            //   </span>
            // )
            console.log(`[DataSourceLoader] Successfully loaded: ${dataSource.name}`)
            continue
          }
          ...
          console.error(`[DataSourceLoader] Failed to load data source ${dataSource.name}:`, error)
          ...
          setDataSources(prev => prev.map(ds =>
            ds.id === dataSource.id
              ? { ...ds, loadingStatus: 'failed' as const, loadingError: errorMessage }
              : ds
          ))
        }
      }
  ...
  }, [connection, initialDataSources, autoCleanupRemovedDataSources, dataSources])

  // Prevent selection event handlers
  const handlePreventSelection = useCallback((e: React.TouchEvent | React.MouseEvent) => {
  ...
  const handleSaveAsFormat = useCallback(async (format: 'csv' | 'json' | 'parquet'): Promise<void> => {
        ...
        case 'csv':
          ...
          if (hasSelections) {
            if (selectedColumns.size > 0) {
              ...
              csvHeaders = sortedColumns.map(index => queryResult.columns[index].name)
              ...
              csvRows = sortedRows.map(rowIndex =>
                queryResult.data[rowIndex] ? queryResult.columns.map(col => {
            ...
            if (selectedColumns.size > 0) {
              ...
              jsonData = queryResult.data.map(row => {
                ...
                sortedColumns.forEach(index => {
                  obj[queryResult.columns[index].name] = row[queryResult.columns[index].name] ?? null
                })
                return obj
              })
              ...
              jsonData = sortedRows.map(rowIndex => {
                ...
                queryResult.columns.forEach(col => {
                  obj[col.name] = queryResult.data[rowIndex]?.[col.name] ?? null
                })
                return obj
              })
        ...
        <Dialog open={showSaveDialog} onOpenChange={setShowSaveDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Choose a format to save your query results. CSV is compatible with spreadsheets and JSON is ideal for web applications.
              </DialogDescription>
            </DialogHeader>
            <div className="py-4">
          ...
          <div className="absolute inset-0 bg-blue-500/10 border-2 border-dashed border-blue-500 rounded-lg flex items-start justify-center pt-20 pointer-events-none z-50">
            <div className="bg-background border border-blue-500 rounded-lg p-6 shadow-lg">
              <div className="flex flex-col items-center gap-2">
                <div className="text-blue-500 font-semibold">Drop to Import Files</div>
