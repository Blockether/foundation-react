...
import { ARROW_MIME_TYPE, CSV_MIME_TYPE, JSONObject, PARQUET_MIME_TYPE } from "./types";
 ...
 */
const runQuery = async (connection: AsyncDuckDBConnection, sql: string): Promise<Table<any>> => {
...
export const exportArrow = async (
    ...
    return new File([buffer as unknown as ArrayBuffer], filename, { type: ARROW_MIME_TYPE });
};
...
export const exportCsv = async (
    ...
    return new File([buffer as unknown as ArrayBuffer], filename, { type: CSV_MIME_TYPE });
};
...
export const exportParquet = async (
    ...
    return new File([buffer as unknown as ArrayBuffer], filename, { type: PARQUET_MIME_TYPE });
};
 ...
 */
export function mapDuckDBTypeToQueryType(duckdbType: string): 'string' | 'number' | 'boolean' | 'date' | 'binary' {
