...
export const useDuckDB = (): {
  ...
  const [state, setState] = React.useState<DuckDBLoadingState>(
    isLoaded()
      ? DuckDBLoadingState.DuckDBLoaded
      : DuckDBLoadingState.DuckDBLoading
  )
...
export const useDuckDBConnection = (
  ...
  >(undefined)
  const [state, setState] = React.useState<DuckDBConnectionState>(
    DuckDBConnectionState.ConnectionClosed
  )
...
export const useDuckDBQuery = (
  ...
  const [state, setState] = React.useState<DuckDBQueryState>(
    isLoaded() ? DuckDBQueryState.QueryIdle : DuckDBQueryState.QueryDBNotStarted
  )
